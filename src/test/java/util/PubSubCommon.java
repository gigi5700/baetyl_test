...
import util.client.fusesource.FuseCallbacks;
...
public class PubSubCommon {
    ...
    public static MqttConnection createMqttConnection(String brokerUrl, String clientId, boolean tls, String certPath,
                                                      MqttConnectOptions connectOptions, boolean authCert, String clientCertPath, String clientKeyPath)
            throws Exception {
        return new MqttConnection(brokerUrl, clientId, tls, certPath, new MemoryPersistence(),
    ...
    public static List<String> publishMessage(CallbackConnection pub, String topic, int msgSize, int start,
                                              int count, QoS qos, boolean retain) throws Exception {
        List<String> pubMessages = new ArrayList<String>();
        ...
        for (int i = start; i < count; i++) {
            String message = String.format("%s_%s", prefix, i);
            pubMessages.add(message);
            publish(pub, topic, qos, message, retain);
            Thread.sleep(200);
        }
