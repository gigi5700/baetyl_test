...
public class PubSubCallback implements MqttCallback {
    ...
    public AtomicInteger getConLostCount() {
        return conLostCount;
    }
    ...
    public Map<String, String> waitAndGetReveiveMap(int expectedCount, long... timeout) throws Exception {
        long waitTimeOut = WAIT_TIME_OUT;
        if (timeout.length > 0) {
            waitTimeOut = timeout[0] * 1000;
        }
        ...
        long start = System.currentTimeMillis();
        while (lastMsgMap.size() < expectedCount && System.currentTimeMillis() - start < waitTimeOut) {
            Thread.sleep(1000);
            log.info("Waiting for msg...");
        }
        return getLastMsgMap();
    }
    ...
    public List<String> getRawReceiveList() {
        return receiveList;
    }

    public List<byte[]> getRawPayloadList() {
        List<byte[]> result = new ArrayList<>();
        rawMqttMsgReceiveList.stream().forEach(msg -> result.add(msg.getPayload()));
        return result;
    }

    public List<String> getRawReceiveListMap(String topic) {
        if (receiveListMap.containsKey(topic)) {
            return receiveListMap.get(topic);
        }
